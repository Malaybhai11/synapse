...
Use any server that implements the OpenAI API format with Synapse. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Synapse can connect to any server using this format.
...
When Synapse runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
4. Update model name in Synapse
```
